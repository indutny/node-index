    ...
    if (item && !item[2]) {
      ...
      step(function() {
        ...
        if (that.storage.isPosition(result)) {
          ...
          splitPage(that.storage, that.order, page, callback); 
        } 
      }));
    } else {
      // Leaf
...
*/
function splitPage(storage, order, page, callback) {
  ...
  if (page.length > order) {
    ...
    step(function() {
      storage.write(page.slice(0, mid_index), this.parallel());
      storage.write(page.slice(mid_index), this.parallel());
